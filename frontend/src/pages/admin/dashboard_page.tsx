...
export class InnerPage extends react.Component<Props, State> {
  ...
  componentDidMount = async () => {
    const [
      rules_info_res,
      total_requests_res,
      total_filtered_requests_res,
      total_active_bans_res,
      templates_res,
      rules_res,
    ] = await Promise.all([
    ...
    if (templates_res.status === 200 && templates_res.data) {
      ...
      mustHaveTemplates = [...data.waf, ...data.jail].filter(
        (t) => t.must_have,
      );
    }
    ...
    this.setState({
      loading: false,
      total_rules:
        rules_info_res.status === 200 ? (rules_info_res.data as any).total : 0,
      total_active_rules:
        rules_info_res.status === 200 ? (rules_info_res.data as any).active : 0,
      total_requests:
        total_requests_res.status === 200
          ? (total_requests_res.data as number)
          : 0,
      total_filtered_requests:
        total_filtered_requests_res.status === 200
          ? (total_filtered_requests_res.data as number)
          : 0,
      total_active_bans:
        total_active_bans_res.status === 200
          ? (total_active_bans_res.data as number)
          : 0,
      mustHaveTemplates,
      appliedMustHave,
      rules,
  ...
  render = () => {
    ...
    const appliedCount = mustHaveTemplates.filter((t) =>
            ...
            description={
              <>
                ...
                <Typography.Title
                  level={4}
                  style={{ margin: 5, cursor: "pointer" }}
                  onClick={() => this.props.navigate("/admin/templates")}
                >
                  {`🔴 ${this.props.t("Security Checklist")}: ${appliedCount}/${totalCount}`}
                </Typography.Title>
              </>
            }
          />
