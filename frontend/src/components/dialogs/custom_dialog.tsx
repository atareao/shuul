...
class InnerDialog<T> extends React.Component<Props<T>, State<T>> {
    ...
    this.state = {
      data:
        this.props.data !== undefined
          ? this.props.data
          : this.props.fields.reduce((acc: any, field: any) => {
  ...
  fetchData = async () => {
    ...
    if (this.props.dialogMode === DialogModes.DELETE) {
      ...
      method = "POST";
      const body = this.props.fields.reduce(
        (acc: any, field: FieldDefinition<T>) => {
    ...
    if (ok) {
      if (this.state.data === undefined) {
        const requiredFields = this.props.fields
          ...
          .join(", ");
        this.showMessage(
          `Los siguientes campos son obligatorios: ${requiredFields}`,
          "error",
        );
        return;
      }
      for (const field of this.props.fields) {
        ...
        if (field.required) {
          if (
            getNestedValue(this.state.data, field.key as string) ===
              undefined ||
            getNestedValue(this.state.data, field.key as string) === null ||
            getNestedValue(this.state.data, field.key as string) === ""
          ) {
  ...
  render = () => {
    ...
    if (dialogMode === DialogModes.CREATE) {
      ...
      message = this.props.t(
        this.props.dialogMessages?.confirmDeleteMessage(data_id),
      ); // Mensaje genérico
    }
    return (
      <>
