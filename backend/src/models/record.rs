...
impl Record {
    ...
    pub async fn create_bulk(pool: &PgPool, records: Vec<NewRecord>) -> Result<Vec<Record>, Error> {
        if records.is_empty() {
            return Ok(Vec::new());
        }
        ...
        let base_sql = "INSERT INTO records (ip_address, protocol,
        ...
        let mut query_builder = query_as::<_, Record>(&full_sql);
        ...
        let created_records = query_builder.fetch_all(&mut *transaction).await?;
    ...
    pub async fn count_paged(pool: &PgPool, params: &ReadRecordParams) -> Result<i64, Error> {
        let filters = vec![
